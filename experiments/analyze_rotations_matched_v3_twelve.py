# DERIVED from analyze_rotations_matched_v3.py on 2026-09-23 17:14; only path constants differ. Source sha256: aec99c0481160e9ff2ea8605a7bc625371ed04bb941d9eca3483a50bcf505d5c
# -*- coding: utf-8 -*-
"""
experiments/analyze_rotations_matched_v3.py
===========================================
Analisi confermativa di rotations_matched_v3, come fissata in
docs/prereg_rotations_block1_vs_block6_emendamento_v3.md (formula del §1 e criterio del §4 di
...
Input:  data/rotations_matched_v3_features.csv  (una riga per PNG, chiave `file`, style + palette)
        data/rotations_matched_v3_manifest.csv
        data/rotations_matched_v3_quality_gate.csv
Output: data/rotations_matched_v3_results.csv          (una riga per spazio)
        data/rotations_matched_v3_prompt_scores.csv    (V(p) e V_scr(p) per prompt e spazio)
        data/rotations_matched_v3_gate_by_condition.csv
...
SHA1 = {"P01": "52e5b19ca6", "P02": "7919c95fcf",
        "S01_oil": "6de4189f5e", "S02_linocut": "e20e86ac75", "S03_cyberpunk": "6ab02b69aa",
...
BENCH_SEEDS = [42, 1337, 4242145]
PROBE_SEEDS = [2718281, 3141592, 1618033]
PROMPT_SEEDS = {p: PROBE_SEEDS if p in ("P01", "P02") else BENCH_SEEDS for p in PROMPTS}
...
def load():
    ...
    feat = pd.read_csv(os.path.join(DATA, "rotations_matched_v3_features.csv"))
    ...
    want = set((p, s, t) for p in PROMPTS for s in PROMPT_SEEDS[p] for t in TAGS)
    if got != want:
...
def main():
    ...
    if (g.n != 36).any():
        raise ValueError(f"cancello: attese 36 immagini per condizione\n{g}")
    ...
    g.to_csv(os.path.join(DATA, "rotations_matched_v3_gate_by_condition.csv"))
